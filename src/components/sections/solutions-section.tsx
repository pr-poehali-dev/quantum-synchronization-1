...
export function SolutionsSection() {
  ...
  return (
    <section id="solutions" className="relative w-full py-16 md:py-24 overflow-hidden">
      <AnimatedBackground variant="gradient" color="rgba(30, 58, 100, 0.06)" secondaryColor="rgba(100, 100, 100, 0.04)" />
            ...
            <ScrollReveal key={solution.title} delay={index * 0.15}>
              <motion.div
                className={`relative h-full rounded-2xl border overflow-hidden flex flex-col ${
                  solution.highlight
                    ? "border-gray-500/40 shadow-[0_0_30px_rgba(100,100,100,0.15)]"
                    : "border-border"
                } bg-card`}
                whileHover={{ y: -4, transition: { duration: 0.3 } }}
              >
                {solution.highlight && (
                  <div className="absolute top-4 right-4 z-10 px-3 py-1 rounded-full bg-gray-800 text-white text-xs font-semibold dark:bg-gray-200 dark:text-gray-900">
                    Популярный выбор
                  </div>
                )}
                ...
                <div className="p-6 flex flex-col flex-1 gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <Icon name={solution.icon} className="h-5 w-5 text-gray-600 dark:text-gray-400" />
                  ...
                  <div className="flex flex-wrap gap-2">
                    {solution.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2.5 py-1 rounded-full bg-gray-500/10 text-gray-600 dark:text-gray-400 text-xs font-medium border border-gray-500/20"
                      >
                        {tag}
                      </span>
                    ))}

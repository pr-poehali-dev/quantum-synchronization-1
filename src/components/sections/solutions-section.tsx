...
export function SolutionsSection() {
  ...
  return (
    <section id="solutions" className="relative w-full py-16 md:py-24 overflow-hidden">
      <AnimatedBackground variant="gradient" color="rgba(30, 58, 100, 0.06)" secondaryColor="rgba(249, 115, 22, 0.04)" />
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {solutions.map((solution, index) => (
            <ScrollReveal key={solution.title} delay={index * 0.15}>
              ...
              >
                {solution.highlight && (
                  <div className="absolute top-4 right-4 z-10 px-3 py-1 rounded-full bg-orange-500 text-white text-xs font-semibold">
                    Популярный выбор
                  </div>
                )}
                ...
                <div className="relative h-48 overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4">
                    <span className="text-5xl font-heading font-bold text-orange-500/20 select-none">
                      {solution.number}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-6 flex flex-col flex-1 gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <Icon name={solution.icon} className="h-5 w-5 text-orange-500" />
                  ...
                  <div className="flex flex-wrap gap-2">
                    {solution.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2.5 py-1 rounded-full bg-orange-500/10 text-orange-500 text-xs font-medium border border-orange-500/20"
                      >
                        {tag}
                      </span>
                    ))}
